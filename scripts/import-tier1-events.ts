#!/usr/bin/env bun
// Import tier1-sites.json events (viva.gr, more.com, gazarte.gr) into database
...
async function main() {
  console.log('📥 Importing tier-1 sites events into database...\n');

  const filePath = join(import.meta.dir, '../data/parsed/tier1-sites.json');

  try {
    const events = JSON.parse(readFileSync(filePath, 'utf-8'));
    console.log(`📂 Found ${events.length} events from tier1-sites.json\n`);
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📊 TIER-1 SITES IMPORT RESULTS:');
    ...
    // Source breakdown is shown in database stats above
